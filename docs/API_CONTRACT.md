...
```ts
{
  ...
  variantGroups: {
    ...
    options: { id: string; name: string; priceDeltaCents: number; stock: number; isActive: boolean }[];
  }[];
}
```
...
```ts
enum OrderStatus { PENDIENTE, PREPARANDO, EN_CAMINO, ENTREGADO, CANCELADO }
```
...
- `409` — negocio no `ACTIVE` o `isOpen:false` ("no acepta pedidos ahora");
  subtotal por debajo de `minOrderCents` del negocio.
...
```ts
{ fcmToken: string }   // va en el body, no en la URL (no queda en logs de acceso)
```
